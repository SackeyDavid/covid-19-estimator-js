const covid19ImpactEstimator = (data) => {
  ...
  function severeImpactFn(params) {
    ...
    objSevereImpact.severeCasesByRequestedTime = Math.floor(
      0.15 * objSevereImpact.infectionsByRequestedTime
    );

    // 35% of total Hosiptal beds are free
    objSevereImpact.hospitalBedsByRequestedTime = Math.floor(
      0.35 * params.totalHospitalBeds -
        objSevereImpact.severeCasesByRequestedTime
    );

    // 5% of infectionsByRequestedTime
    objSevereImpact.casesForICUByRequestedTime = Math.floor(
      0.05 * objSevereImpact.infectionsByRequestedTime
    );

    // 2% of infectionsByRequestedTime
    objSevereImpact.casesForVentilatorsByRequestedTime = Math.floor(
      0.02 * objSevereImpact.infectionsByRequestedTime
    );
